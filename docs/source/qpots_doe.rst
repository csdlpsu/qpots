...
The visualization shows task-specific observations for Branin and Currin, the
...
``python -m examples.decoupled_branin_currin_figure`` with ``python main.py``.
See :ref:`Running the repository examples <running-the-repository-examples>`.

.. note::

   qPOTS uses stochastic posterior sampling and evolutionary optimization.
   Consequently, results generated by this example may differ slightly from
